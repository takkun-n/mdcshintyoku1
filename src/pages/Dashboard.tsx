...
const Dashboard: React.FC = () => {
  ...
  const handleDateRangeChange = (startDate: string, endDate: string) => {
    ...
    const filtered = productionData.filter(item => {
      ...
      if (startDate && endDate) {
        ...
        end.setHours(23, 59, 59); // 終了日の終了時刻
  ...
  // 集計用の計算
  const totalPlanned = filteredData.reduce((sum, item) => sum + item.plannedQuantity, 0);
  ...
  const totalHours = filteredData.reduce((sum, item) => sum + item.workHours, 0);
